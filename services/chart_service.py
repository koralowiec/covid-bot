...
class ChartService:
    ...
    @staticmethod
    def grouped_bar_chart(labels,legend: str =[],to_group_1: int = [] ,to_group_2 : int = [],to_group_3: int = []):
        x = np.arange(len(labels))
        width = 0.3
        fig,ax=plt.subplots()
        if to_group_1 : plt.bar(x-0.2, to_group_1, width, color='cyan')
        if to_group_2 : plt.bar(x, to_group_2, width, color='orange')
        if to_group_3 : plt.bar(x+0.2, to_group_3, width, color='red')
    ...
    def plot_latest_voivodeships(cls):
        ...
        infected = [ daily_infected.daily.infected for daily_infected in record.voivodeships]
    ...
    @classmethod
    def plot_n_latest_records(cls,n: int = 7,voivodeships: str = None,config: str = "total"):
        record = cls.record_service.get_n_latest_records(n)
        labels = [datetime.strptime(date.date, '%d.%m.%Y %H:%M').strftime('%m/%d/%Y') for date in record ]
        if voivodeships:
            infected_to_format = [[ x.daily['infected'] if x.name == voivodeships else 0 for x in voievode.voivodeships] for voievode in record]
            tested_to_format = [[ x.daily['tested'] if x.name == voivodeships else 0 for x in voievode.voivodeships] for voievode in record]
            #Format the value
            ...
            return cls.grouped_bar_chart(labels,cls.config[1],dead,infected,recovered)
